...
- Проверка соединения с БД и работы web-приложения
    ...
    - Смотрим работу ```docker ps ``` (после деплоя Docker образа в Registry потребуется некоторое время, пока app.service запустит docker compose, но не более 3-5 минут)
